...
import sys
import requests
from cfg.settings import LLM_API_URL, MODEL_NAME, API_PAYLOAD, HEADERS
from prompt.llm_prompt import FEATURE_ENTITY_PROMPT
from cfg.settings import LLM_POST_SIMILAR_PATH, FINAL_PATH
...
def process_single_line(line, index):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if not feature:
                print(f"'对应特征'字段值为空 [行 {index}]: {str(line)[:50]}...")
                return index, None
