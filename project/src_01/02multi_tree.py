...
from cfg.settings import JSON_ORI_PATH, MULTI_TREE_PATH
...
# if __name__ == "__main__":
#     # 确保输出文件存在
#     file_path = Path(MULTI_TREE_PATH)
#     file_path.parent.mkdir(parents=True, exist_ok=True)  # 确保目录存在
#     file_path.touch(exist_ok=True)  # 创建文件（如果不存在）
#     print(f"输出文件已准备: {file_path}")
...
#         print("\n==== 正在写入所有数据结构 ====")
#         with open(MULTI_TREE_PATH, "w", encoding="utf-8") as f:
#             for i, result_dict in enumerate(data_results):
#                 f.write(json.dumps(result_dict, ensure_ascii=False) + "\n")
#         print(f"所有数据已成功写入: {MULTI_TREE_PATH}")
